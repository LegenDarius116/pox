...
class SH_LoadBalancing():
  ...
  def __init__ (self, connection,service_ip, servers=[], alg="sourcehashing", arp_timer=0, lldp_timer=1):
    ...
    if arp_timer==SH_LoadBalancing.ARP_TIMER and lldp_timer==SH_LoadBalancing.LLDP_TIMER:
        log.debug("You have set arp_timer and lldp_timer parameters to 1. Both parameter cannot be 1, change one parameter to 0 on the launch function.")
        return
  ...
  def add_sysflow(self, machine, dir, idleTime, hardTime, machine2 = None):
    ...
    if dir == 0:
        ...
        if clientip not in self.sys_flow[self.con.dpid]:
            self.sys_flow[self.con.dpid][clientip]= SysFlow(machine, [destinationtMachine], dir, idleTime, hardTime)
  ...
  def _handle_PacketIn (self, event):
    ...
    #tcp packet
            ...
            if httpResponseExists == False:
                ...
                if destinationip in self.sys_flow[self.con.dpid]:
                    ...
                    if sysFlowSrc.dir == 0:
                        ...
                        sys_flow = self.add_sysflow(sourceMachine,1, self.flow_idle_time, self.flow_hard_time, clientMachine)                        
        ...
        #handles http request
            ...
            actions.append(of.ofp_action_dl_addr.set_dst(destination.mac.toStr())) #dst server to handle request: mac
            actions.append(of.ofp_action_nw_addr.set_src(sourceip))
            actions.append(of.ofp_action_nw_addr.set_dst(destination.ip.toStr()))    #dst server to handle request: ip            
...
def launch (reactive = False):
    ...
    def loadbalancing_start (event):
        ...
        core.registerNew(SH_LoadBalancing, event.connection, service_ip="192.168.0.11", servers=["192.168.0.64","192.168.0.128","192.168.0.192","192.168.0.254"], alg="sourcehashing", arp_timer=1, lldp_timer=0)
